...
public class FastestWeightingTest {
    ...
    public void testSpeed0() {
        Weighting instance = new FastestWeighting(encoder);
        assertEquals(1.0 / 0, instance.calcWeight(createMockedEdgeIteratorState(10,
                GHUtility.setProperties(encodingManager.createEdgeFlags(), encoder, 0, true, true)),
                false, EdgeIterator.NO_EDGE), 1e-8);

        // 0 / 0 returns NaN but calcWeight should not return NaN!
        assertEquals(1.0 / 0, instance.calcWeight(createMockedEdgeIteratorState(0,
                GHUtility.setProperties(encodingManager.createEdgeFlags(), encoder, 0, true, true)),
                false, EdgeIterator.NO_EDGE), 1e-8);
    }
