...
public class Bike2WeightFlagEncoderTest extends BikeFlagEncoderTest {
    ...
    public void testSetSpeed0_issue367() {
        IntsRef flags = GHUtility.setProperties(encodingManager.createEdgeFlags(), encoder, 10, 15);
        assertEquals(10, encoder.getSpeed(flags), .1);
        assertEquals(10, encoder.getReverseSpeed(flags), .1);

        encoder.getAverageSpeedEnc().setDecimal(false, flags, 0);
    ...
    public void testRoutingFailsWithInvalidGraph_issue665() {
        ...
        IntsRef wayFlags = encodingManager.handleWayTags(way, includeWay, relationFlags);
