...
public class EncodingManager implements EncodedValueLookup {
    ...
    public EncodingManager(List<? extends FlagEncoder> flagEncoders, int bytesForEdgeFlags) {
        ...
        addEncodedValue(new BooleanEncodedValue("roundabout", false));
...
//            System.out.println(ev.getName() + " " + config.getRequiredBits());
    ...
    public List<FlagEncoder> getEncodersFromNodeFlags(long importNodeFlags) {
        List<FlagEncoder> list = new ArrayList<>(edgeEncoders.size());
        for (int i = 0; i < edgeEncoders.size(); i++) {
            FlagEncoder encoder = edgeEncoders.get(i);
            if (((1L << (i + 1)) & importNodeFlags) != 0)
                list.add(encoder);
        }
        return list;
    }
